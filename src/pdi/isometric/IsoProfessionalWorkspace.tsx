...
export interface IsoProfessionalWorkspaceProps { projectName?: string; }

/** Professional drawing-session shell around the proven V4.8d engine. */
export default function IsoProfessionalWorkspace({ projectName = "Nouveau projet" }: IsoProfessionalWorkspaceProps) {
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      ...
      if (key === "escape") { setLayersOpen(false); setPropertiesOpen(false); }
    };
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#0b0e11] text-slate-100">
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <main className="relative min-w-0 flex-1 overflow-hidden"><PdiIsometricEditor />{grid && <div className="pointer-events-none absolute inset-0 opacity-[0.025]" style={{ backgroundImage: "linear-gradient(#94a3b8 1px, transparent 1px), linear-gradient(90deg, #94a3b8 1px, transparent 1px)", backgroundSize: "24px 24px" }} />}</main>
        {propertiesOpen && <aside className="hidden w-64 shrink-0 border-l border-slate-800 bg-[#11151a] xl:block"><div className="border-b border-slate-800 px-3 py-3"><div className="text-[10px] font-semibold uppercase tracking-[0.16em] text-slate-500">Propriétés</div></div><div className="space-y-4 p-3 text-[10px] text-slate-500"><div><div className="mb-1 text-slate-600">Outil actif</div><div className="rounded-md border border-slate-800 bg-[#0c0f12] px-2.5 py-2 text-slate-300">{TOOLS.find((item) => item.id === tool)?.label}</div></div><div><div className="mb-1 text-slate-600">Affichage</div><div className="rounded-md border border-slate-800 bg-[#0c0f12] px-2.5 py-2 text-slate-300">Grille {grid ? "activée" : "désactivée"}</div></div><div className="rounded-md border border-slate-800 bg-[#0c0f12] p-2.5 leading-5">Les propriétés de l'objet sélectionné seront reliées au modèle ISO canonique dans l'étape suivante.</div></div></aside>}
      ...
      <footer className="flex h-7 shrink-0 items-center justify-between border-t border-slate-800 bg-[#0a0d10] px-3 text-[9px] text-slate-600"><div className="flex items-center gap-4"><span className="text-emerald-500">● Prêt</span><span>Outil : {TOOLS.find((item) => item.id === tool)?.label}</span><span>Grille : {grid ? "ON" : "OFF"}</span></div><div className="flex items-center gap-4"><span>{saved ? "Enregistré" : "Modifications non enregistrées"}</span><span>ISO V4.8d</span></div></footer>
    </div>
  );
}
