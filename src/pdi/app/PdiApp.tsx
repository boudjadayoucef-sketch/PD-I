    ...
    <div className="flex h-screen min-h-[680px] flex-col overflow-hidden bg-[#0c0f12] font-sans text-slate-100">
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-slate-800 bg-[#0d1014] px-3">
        <div className="flex min-w-0 items-center gap-3">
          <button onClick={() => openWorkspace("accueil")} className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-slate-800" title="Accueil PD&I">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setCommandOpen(true)} className="hidden items-center gap-2 rounded-md border border-slate-800 bg-slate-900 px-3 py-1.5 text-xs text-slate-500 hover:text-slate-300 md:flex">
          ...
          <div className="relative">
            <button onClick={() => setAccountOpen((v) => !v)} className="rounded-md border border-slate-700 px-2.5 py-1.5 text-xs hover:bg-slate-800">Compte</button>
            {accountOpen && (
      ...
      <div className="flex h-11 shrink-0 items-center border-b border-slate-800 bg-[#12161b] px-3">
        <div className="flex h-full items-center gap-1">
          {MODES.map((item) => (
            <button key={item.id} onClick={() => setMode(item.id)} className={`h-8 rounded-md px-4 text-[10px] font-semibold tracking-wide transition ${mode === item.id ? "bg-cyan-500/15 text-cyan-300 ring-1 ring-cyan-500/30" : "text-slate-500 hover:bg-slate-800 hover:text-slate-300"}`}>
              {item.label}
            </button>
          ))}
        </div>
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="flex w-14 shrink-0 flex-col items-center border-r border-slate-800 bg-[#0e1216] py-2">
          {WORKSPACES.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => openWorkspace(item.id)} title={item.hint} className={`group mb-1 flex w-12 flex-col items-center rounded-md py-2 transition ${active ? "bg-cyan-500/10 text-cyan-300" : "text-slate-600 hover:bg-slate-800 hover:text-slate-300"}`}>
        ...
        <main className="min-w-0 flex-1 overflow-hidden">
          {workspace === "isometries" ? (
            <div className="h-full overflow-hidden">
              <IsoWorkspace projectName="Nouveau projet" />
            </div>
          ) : workspace === "accueil" ? (
